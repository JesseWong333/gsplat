...
class GaussianSSC(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.drop_out = kwargs.get("dropout", 1.0)
        ...
        self._opacity = nn.Parameter(torch.rand(self.num_blocks * self.num_points_per_block, 1))
    ...
    def forward(self, x):
        ...
        # 进行 dropout？
        if self.training:
            drop_prob = self.drop_out
        else:
            drop_prob = 1.0
        num_points = int(self.num_blocks * self.num_points_per_block * drop_prob)
        indices = torch.randint(0, self.num_blocks * self.num_points_per_block, (num_points,))
        xyz_droped  = self.get_xyz[indices]
        scaling_droped = self.get_scaling[indices]
        rotation_droped = self.get_rotation[indices]
        ...
        xys, depths, radii, conics, num_tiles_hit = project_gaussians(xyz_droped, scaling_droped, 1, 
                                                                                       rotation_droped, self.H, self.W, self.L,
                                                                                            self.tile_bounds)
...
if __name__ == '__main__':
    ...
    gaussian_model = GaussianSSC(Gaussian_points_per_block=Gaussian_points_per_block, 
                                 ...
                                 lidar_mins=mins, dropout=1.0).cuda()
    ...
    optimizer = torch.optim.Adam(param_groups_weight_decay(gaussian_model, weight_decay=0.), lr=0.001)
    ...
    targetes = torch.from_numpy(block_values).reshape(-1).cuda().float()
...
# 8.8 结论
# 用 dropout 会减少一部分过大的 scale，这时 loss偏大，需要用更多的高斯点
# 还没测试在推理阶段不dropout的loss

# 用大学习率，0.01会使得半径增加更快一些，比如达到 4000
# 0.001 可达到 8000

start_time = time.time()
gaussian_model.eval()
outputs = gaussian_model.forward(batch_sample_points)# N * 16 * 16 * 16

loss = loss_fn(outputs, targetes)
print("test_Loss:", loss.item())

outputs[outputs > 0] = 1
outputs[outputs <= 0] = 0
rendering_result = outputs
